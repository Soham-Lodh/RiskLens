    ...
    >
      ...
      <div
        style={{
          ...
          height: 600,
          background:
            "radial-gradient(circle, rgba(252,163,17,0.07) 0%, transparent 70%)",
          pointerEvents: "none",
        }}
      />
      ...
      >
        ...
        >
          ML-powered probability engine with 98.34% AUC accuracy. Assess
          applicant default risk, generate credit scores, and classify portfolio
          exposure — in seconds.
        </p>
...
function Features() {
  ...
  return (
    <section
      id="features"
      style={{ background: "#000", padding: "100px 40px" }}
    >
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div ref={ref} style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <p
            className={`fade-up delay-1${inView ? " visible" : ""}`}
            style={{
              fontSize: 16,
              color: "rgba(255,255,255,0.4)",
              maxWidth: 520,
          ...
          >
            Production ML model validated on real lending data with
            industry-leading discrimination metrics.
          </p>
        </div>
        <div
          style={{
          ...
          {WORKFLOW_STEPS.map((step, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center" }}>
              ...
              {i < WORKFLOW_STEPS.length - 1 && (
                <div
                  className={`fade-in delay-${i + 1}${inView ? " visible" : ""}`}
                  style={{
                    width: 32,
                    textAlign: "center",
                    color: "rgba(252,163,17,0.4)",
                    fontSize: 18,
                    marginBottom: 24,
                    flexShrink: 0,
                  }}
          ...
          {[
            {
              ...
              val: "94%",
              sub: "Recall on defaults",
            },
            {
              label: "False Positive Rate",
              val: "7%",
              sub: "Non-default misclass.",
            },
            { label: "Weighted F1", val: "0.94", sub: "Balanced performance" },
            {
              label: "KS Statistic",
              val: "~0.87",
              sub: "Discriminatory power",
            },
          ].map((s, i) => (
            <div
              key={i}
...
function FieldGroup({ label, hint, error, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      {hint && !error && (
        <div
          style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginTop: 5 }}
        >
          {hint}
        </div>
      )}
...
function validateStep(step, form) {
  ...
  if (step === 1) {
    ...
    if (!form.loan_amount || form.loan_amount <= 0)
      errs.loan_amount = "Enter loan amount";
    if (!form.loan_tenure_months || form.loan_tenure_months <= 0)
      errs.loan_tenure_months = "Enter tenure";
    if (!form.num_open_accounts || form.num_open_accounts < 0)
      errs.num_open_accounts = "Required";
  }
  if (step === 2) {
    if (
      form.credit_utilization_ratio === "" ||
      form.credit_utilization_ratio < 0 ||
      form.credit_utilization_ratio > 100
    )
      errs.credit_utilization_ratio = "Enter 0–100";
    if (!form.total_loan_months || form.total_loan_months <= 0)
      errs.total_loan_months = "Required";
    if (form.delinquent_months === "" || form.delinquent_months < 0)
      errs.delinquent_months = "Required";
    if (form.total_dpd === "" || form.total_dpd < 0)
      errs.total_dpd = "Required";
  }
  if (step === 3) {
...
function FormStep0({ form, set, errs }) {
  return (
    <div>
      <FieldGroup
        label="Age"
        hint="Applicant must be 18 or older"
        error={errs.age}
      >
        <CInput
          value={form.age}
      ...
      <FieldGroup
        ...
        hint="Current living arrangement"
        error={errs.residence_type}
      >
        <CSelect
...
function FormStep1({ form, set, errs }) {
  return (
    <div>
      <FieldGroup
        label="Annual Income (₹)"
        hint="Gross yearly income before tax"
        error={errs.income}
      >
        <CInput
          value={form.income}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <FieldGroup
          ...
          error={errs.loan_tenure_months}
        >
          <CInput
            value={form.loan_tenure_months}
            onChange={(v) => set("loan_tenure_months", v)}
            placeholder="e.g. 36"
            min={1}
            error={errs.loan_tenure_months}
          />
        ...
        <FieldGroup
          label="Open Accounts"
          hint="Active credit accounts"
          error={errs.num_open_accounts}
        >
          <CInput
            value={form.num_open_accounts}
            onChange={(v) => set("num_open_accounts", v)}
            placeholder="e.g. 3"
            min={0}
            error={errs.num_open_accounts}
          />
  ...
  return (
    <div>
      <FieldGroup
        ...
        hint="Percentage of available credit currently used"
        error={errs.credit_utilization_ratio}
      >
        <CInput
          value={form.credit_utilization_ratio}
          onChange={(v) => set("credit_utilization_ratio", v)}
          placeholder="e.g. 42"
          min={0}
          max={100}
          step={0.1}
      ...
      <FieldGroup
        ...
        hint="Cumulative months of all loan histories"
        error={errs.total_loan_months}
      >
        <CInput
          value={form.total_loan_months}
          onChange={(v) => set("total_loan_months", v)}
          placeholder="e.g. 60"
          min={1}
          error={errs.total_loan_months}
        />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <FieldGroup
          label="Delinquent Months"
          hint="Months with missed payments"
          error={errs.delinquent_months}
        >
          <CInput
            value={form.delinquent_months}
            onChange={(v) => set("delinquent_months", v)}
            placeholder="e.g. 2"
            min={0}
            error={errs.delinquent_months}
          />
        ...
        <FieldGroup
          label="Total DPD"
          hint="Sum of days past due"
          error={errs.total_dpd}
        >
          <CInput
            value={form.total_dpd}
            onChange={(v) => set("total_dpd", v)}
            placeholder="e.g. 15"
            min={0}
            error={errs.total_dpd}
          />
...
function FormStep3({ form, set, errs }) {
  return (
    <div>
      <FieldGroup
        label="Loan Purpose"
        hint="Primary reason for borrowing"
        error={errs.loan_purpose}
      >
        <CSelect
      ...
      <FieldGroup
        label="Loan Type"
        hint="Collateralized vs uncollateralized"
        error={errs.loan_type}
      >
        <CSelect
...
function PredictionForm({ onResult }) {
          ...
          {ANALYSIS_STAGES.map((s, i) => (
            ...
            >
              ...
              <span
                style={{
                  fontSize: 13,
                  color: i <= stage ? "#fff" : "rgba(255,255,255,0.2)",
                }}
              >
                {s}
              </span>
  ...
  return (
    <div>
      ...
      <div style={{ minHeight: 260 }}>{STEP_COMPONENTS[step]}</div>
...
function AnimatedScore({ score }) {
  ...
  return <span style={{ fontFamily: "JetBrains Mono, monospace" }}>{val}</span>;
}
  ...
  return (
    <div ref={ref} style={{ maxWidth: 1000, margin: "0 auto" }}>
      ...
      >
        ...
        >
          Applicant Age {form.age} · {form.residence_type} · {form.loan_purpose}{" "}
          Loan
        </p>
      </div>
      ...
      >
        ...
        {/* Rating */}
        <div
          className={`fade-up delay-3${inView ? " visible" : ""}`}
          style={{
            background: "rgba(20,33,61,0.6)",
            border: "1px solid rgba(255,255,255,0.07)",
            borderRadius: 16,
      ...
      {/* KPI row */}
      <div
        className={`fade-up delay-4${inView ? " visible" : ""}`}
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(4,1fr)",
          gap: 16,
        ...
        {[
          {
            ...
            val:
              (Number(form.loan_amount) / Number(form.income)).toFixed(2) + "×",
            sub: "Leverage Ratio",
          },
          {
            label: "Utilization",
            val: Number(form.credit_utilization_ratio).toFixed(1) + "%",
            sub: "Credit Usage",
          },
          {
            label: "Delinquency",
            val:
              (
                (Number(form.delinquent_months) /
                  Number(form.total_loan_months)) *
                100
              ).toFixed(1) + "%",
            sub: "Default Rate",
          },
          {
            label: "Avg DPD",
            val:
              form.delinquent_months > 0
                ? (
                    Number(form.total_dpd) / Number(form.delinquent_months)
                  ).toFixed(1)
                : "0",
            sub: "Days Past Due",
          },
      ...
      <div
        className={`fade-up delay-5${inView ? " visible" : ""}`}
        style={{
          display: "grid",
        ...
        >
          ...
          >
            {[
              {
                k: "Amount",
                v: `₹${Number(form.loan_amount).toLocaleString("en-IN")}`,
              },
              {
                k: "Income",
                v: `₹${Number(form.income).toLocaleString("en-IN")}`,
              },
      ...
      >
        This assessment is generated by an ML model for analytical purposes
        only. Results should be used in conjunction with full underwriting
        review and do not constitute a lending decision.
      </div>
    </div>
  );
}
        ...
        {!result ? (
          <>
            <div style={{ textAlign: "center", marginBottom: 48 }}>
              ...
              >
                Complete the assessment form. Our ML model will analyze 12+ risk
                indicators and return a probability score, credit rating, and
                risk classification.
              </p>
            </div>
